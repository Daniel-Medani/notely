    ...
    <Dialog open={open} onOpenChange={handleOpenChange} aria-label="Search pages">
      ...
      >
        ...
        <DialogDescription className="sr-only">
          Search through your workspace pages
        </DialogDescription>
        {/* Input area */}
        <div
          className={`flex items-center gap-2 px-2 py-1 ${loading || error || query.length >= 2 ? 'border-b' : ''}`}
        >
        ...
        {(loading || error || query.length >= 2) && (
          <div role="listbox" className="max-h-[352px] overflow-y-auto py-2">
            {loading && (
              <>
                <Skeleton
                  className="h-11 w-full rounded-md mx-4 my-1"
            ...
            {!loading && !error && results.length === 0 && query.length >= 2 && (
              <div className="flex flex-col items-center justify-center px-4 py-8 gap-1">
                <p className="text-sm text-foreground">
                  No results for &ldquo;{query.length > 40 ? query.slice(0, 40) + '...' : query}
                  &rdquo;
                </p>
